...
const registration = async (req, res) => {
  ...
  try {
    ...
    if (!password)
      return res.status(400).send({ error: "Passord is required!" });
    ...
    res
      .status(201)
      .send({ success: "Registration susseccfull! Please verify your email." });
  } catch (error) {
...
const verifyEmailAddress = async (req, res) => {
  ...
  try {
    if (!email || !otp)
      return res.status(400).send({ error: "Invalid reqest!" });
...
const loginController = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ success: "Login Sussessfull", user: loggedUser, accessToken });
  } catch (error) {
    res.status(500).send({ error: "Server error!" });
  }
};
...
const update = async (req, res) => {
  ...
  try {
    ...
    if (fullName)
      existingUser.fullName = fullName.trim().split(/\s+/).join(" ");
